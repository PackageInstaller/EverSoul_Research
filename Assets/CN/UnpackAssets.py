...
def save_image(args: Tuple) -> Tuple[bool, str, str]:
    img, output_path, name, subdir, add_suffix = args
    try:
    ...
    for env in loaded_bundles:
        for asset in env.objects:
            ...
            try:
                ...
                img = data.image
                if task.add_suffix:
                    output_path = output_dir / f"{name} (2).png"
                else:
                    output_path = output_dir / f"{name}.png"
                export_tasks.append((img, str(output_path), name, task.output_subdir, task.add_suffix))
...
def main():
    ...
    tasks = [
        ...
        BundleTask(
            # ui_assets_assets/bundles/ui/textures/souls
            pattern="ui_zl_assets_assets/bundles_zl/ui/textures/souls/*.bundle",
            output_subdir="soul",
            asset_type="Texture2D",
            description="soul",
            add_suffix=True,
        ...
        BundleTask(
            # ui_zl_assets_assets/bundles_zl/ui/textures/souls
