...
namespace
{
    ...
    struct AppConfig
    {
        static constexpr std::string_view LIVE_TABLE_DIR = "../live_table";
        static constexpr std::string_view SCHEMA_DIR = "../schema";
    ...
    auto processReviewServer(AppState &state) -> bool
    {
        try
        {
            auto reviewInfo = TableUpdater::checkReviewServer(state.version);

            if (reviewInfo.exists)
            {

                if (TableUpdater::downloadAndProcessReviewTables(reviewInfo))
                {
    ...
    auto processLiveTables(AppState &state) -> bool
    {
        try
        {
            ...
            {
                if (TableUpdater::checkAndUpdateLiveTables(state.version))
                {
                    std::println("Live数据表下载完成");
                    state.needGenerateApis = true;
                }
                else
                {
