...
def main():
    ...
    parser.add_argument('-o', '--output_dir', help='输出目录的基础路径(可选)', default=None)
    ...
    for proto_type in proto_types:
        ...
        if args.output_dir is None:
            # 根据命名空间选择默认输出目录
            if proto_type == "EsPb":
                output_dir = "../receive_proto"
            else:  # ReqPb
                output_dir = "../request_proto"
        else:
            # 如果指定了基础输出目录，则在其下创建子目录
            output_dir = os.path.join(args.output_dir, proto_type.lower())
        ...
        print(f"\n开始处理 {proto_type} 类型...")
        process_dump_file(proto_type, args.input_file, output_dir)
