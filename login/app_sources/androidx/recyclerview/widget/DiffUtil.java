...
public class DiffUtil {
    private static final Comparator<Snake> SNAKE_COMPARATOR = new Comparator<Snake>() { // from class: androidx.recyclerview.widget.DiffUtil.1
        ...
        public int compare(Snake snake, Snake snake2) {
            int i = snake.f130x - snake2.f130x;
            return i == 0 ? snake.f131y - snake2.f131y : i;
        }
    };
    ...
    public static DiffResult calculateDiff(Callback callback, boolean z) {
        ...
        while (!arrayList2.isEmpty()) {
            ...
            if (diffPartial != null) {
                ...
                diffPartial.f130x += range.oldListStart;
                diffPartial.f131y += range.newListStart;
                Range range2 = arrayList3.isEmpty() ? new Range() : (Range) arrayList3.remove(arrayList3.size() - 1);
                range2.oldListStart = range.oldListStart;
                range2.newListStart = range.newListStart;
                if (diffPartial.reverse) {
                    range2.oldListEnd = diffPartial.f130x;
                    range2.newListEnd = diffPartial.f131y;
                } else if (diffPartial.removal) {
                    range2.oldListEnd = diffPartial.f130x - 1;
                    range2.newListEnd = diffPartial.f131y;
                } else {
                    range2.oldListEnd = diffPartial.f130x;
                    range2.newListEnd = diffPartial.f131y - 1;
                }
                arrayList2.add(range2);
                if (diffPartial.reverse) {
                    if (diffPartial.removal) {
                        range.oldListStart = diffPartial.f130x + diffPartial.size + 1;
                        range.newListStart = diffPartial.f131y + diffPartial.size;
                    } else {
                        range.oldListStart = diffPartial.f130x + diffPartial.size;
                        range.newListStart = diffPartial.f131y + diffPartial.size + 1;
                    }
                } else {
                    range.oldListStart = diffPartial.f130x + diffPartial.size;
                    range.newListStart = diffPartial.f131y + diffPartial.size;
    ...
    */
    ...
    public static class DiffResult {
        ...
        private void addRootSnake() {
            ...
            if (snake != null && snake.f130x == 0 && snake.f131y == 0) {
                return;
            }
        ...
        private void findMatchingItems() {
            ...
            for (int size = this.mSnakes.size() - 1; size >= 0; size--) {
                ...
                int i3 = snake.f130x + snake.size;
                int i4 = snake.f131y + snake.size;
                ...
                for (int i5 = 0; i5 < snake.size; i5++) {
                    int i6 = snake.f130x + i5;
                    int i7 = snake.f131y + i5;
        ...
        private boolean findMatchingItem(int i, int i2, int i3, boolean z) {
            ...
            while (i3 >= 0) {
                ...
                int i7 = snake.f130x + snake.size;
                int i8 = snake.f131y + snake.size;
                if (z) {
        ...
        public void dispatchUpdatesTo(ListUpdateCallback listUpdateCallback) {
            ...
            for (int size = this.mSnakes.size() - 1; size >= 0; size--) {
                Snake snake = this.mSnakes.get(size);
                int i3 = snake.size;
                int i4 = snake.f130x + i3;
                int i5 = snake.f131y + i3;
                if (i4 < i) {
                ...
                for (int i6 = i3 - 1; i6 >= 0; i6--) {
                    if ((this.mOldItemStatuses[snake.f130x + i6] & 31) == 2) {
                        batchingListUpdateCallback.onChanged(snake.f130x + i6, 1, this.mCallback.getChangePayload(snake.f130x + i6, snake.f131y + i6));
                    }
                }
                i = snake.f130x;
                i2 = snake.f131y;
