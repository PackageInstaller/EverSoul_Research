...
class FragmentStateManager {
    ...
    int computeExpectedState() {
        ...
        int i2 = C04472.$SwitchMap$androidx$lifecycle$Lifecycle$State[this.mFragment.mMaxState.ordinal()];
        if (i2 != 1) {
            if (i2 == 2) {
                i = Math.min(i, 5);
            } else if (i2 == 3) {
    ...
    void ensureInflatedView() {
        if (this.mFragment.mFromLayout && this.mFragment.mInLayout && !this.mFragment.mPerformedCreateView) {
            ...
            if (this.mFragment.mView != null) {
                ...
                this.mFragment.mView.setTag(C0398R.id.fragment_container_view_tag, this.mFragment);
                if (this.mFragment.mHidden) {
    ...
    void resume() {
        ...
        if (focusedView != null && isFragmentViewChild(focusedView)) {
            ...
            if (FragmentManager.isLoggingEnabled(2)) {
                Log.v(TAG, "requestFocus: Restoring focused view " + focusedView + StringUtils.SPACE + (requestFocus ? AnalyticsEvents.PARAMETER_SHARE_OUTCOME_SUCCEEDED : "failed") + " on Fragment " + this.mFragment + " resulting in focused view " + this.mFragment.mView.findFocus());
            }
        }
