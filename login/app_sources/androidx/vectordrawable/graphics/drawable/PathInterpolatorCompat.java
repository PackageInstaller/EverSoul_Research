...
public class PathInterpolatorCompat implements Interpolator {
    ...
    /* renamed from: mX */
    private float[] f139mX;

    /* renamed from: mY */
    private float[] f140mY;
    ...
    private void initPath(Path path) {
        ...
        this.f139mX = new float[min];
        this.f140mY = new float[min];
        float[] fArr = new float[2];
        for (int i2 = 0; i2 < min; i2++) {
            pathMeasure.getPosTan((i2 * length) / (min - 1), fArr, null);
            this.f139mX[i2] = fArr[0];
            this.f140mY[i2] = fArr[1];
        }
        if (Math.abs(this.f139mX[0]) <= 1.0E-5d && Math.abs(this.f140mY[0]) <= 1.0E-5d) {
            int i3 = min - 1;
            if (Math.abs(this.f139mX[i3] - 1.0f) <= 1.0E-5d && Math.abs(this.f140mY[i3] - 1.0f) <= 1.0E-5d) {
                float f = 0.0f;
                int i4 = 0;
                while (i < min) {
                    float[] fArr2 = this.f139mX;
                    int i5 = i4 + 1;
                    float f2 = fArr2[i4];
                    if (f2 < f) {
        ...
        int i6 = min - 1;
        throw new IllegalArgumentException("The Path must start at (0,0) and end at (1,1) start: " + this.f139mX[0] + "," + this.f140mY[0] + " end:" + this.f139mX[i6] + "," + this.f140mY[i6]);
    }
    ...
    public float getInterpolation(float f) {
        ...
        int length = this.f139mX.length - 1;
        int i = 0;
        while (length - i > 1) {
            int i2 = (i + length) / 2;
            if (f < this.f139mX[i2]) {
        ...
        float[] fArr = this.f139mX;
        float f2 = fArr[length];
        float f3 = fArr[i];
        float f4 = f2 - f3;
        if (f4 == 0.0f) {
            return this.f140mY[i];
        }
        float f5 = (f - f3) / f4;
        float[] fArr2 = this.f140mY;
        float f6 = fArr2[i];
