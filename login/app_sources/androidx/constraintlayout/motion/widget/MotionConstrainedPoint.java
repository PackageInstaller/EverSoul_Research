...
class MotionConstrainedPoint implements Comparable<MotionConstrainedPoint> {
    ...
    /* JADX WARN: Unreachable blocks removed: 1, instructions: 1 */
    void different(MotionConstrainedPoint points, boolean[] mask, String[] custom2) {
        mask[0] = mask[0] | diff(this.position, points.position);
        mask[1] = mask[1] | diff(this.f56x, points.f56x);
        mask[2] = mask[2] | diff(this.f57y, points.f57y);
    ...
    void fillStandard(double[] data, int[] toUse) {
        float[] fArr = {this.position, this.f56x, this.f57y, this.width, this.height, this.alpha, this.elevation, this.rotation, this.rotationX, this.rotationY, this.scaleX, this.scaleY, this.mPivotX, this.mPivotY, this.translationX, this.translationY, this.translationZ, this.mPathRotate};
        int i = 0;
        for (int i2 : toUse) {
            if (i2 < 18) {
                data[i] = fArr[r4];
