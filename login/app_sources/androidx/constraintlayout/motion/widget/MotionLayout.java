...
public class MotionLayout extends ConstraintLayout implements NestedScrollingParent3 {
    ...
    void setState(TransitionState newState) {
        ...
        int i = C01915.f58xabc7e4ac[transitionState.ordinal()];
        if (i == 1 || i == 2) {
    ...
    static /* synthetic */ class C01915 {
        ...
        static {
            ...
            try {
                f58xabc7e4ac[TransitionState.SETUP.ordinal()] = 2;
            } catch (NoSuchFieldError unused2) {
            }
            try {
                f58xabc7e4ac[TransitionState.MOVING.ordinal()] = 3;
            } catch (NoSuchFieldError unused3) {
            }
            try {
                f58xabc7e4ac[TransitionState.FINISHED.ordinal()] = 4;
    ...
    private static class MyTracker implements MotionTracker {
        ...
        /* renamed from: me */
        private static MyTracker f59me = new MyTracker();
        ...
        public static MyTracker obtain() {
            f59me.tracker = VelocityTracker.obtain();
            return f59me;
        }
    ...
    */
    ...
    private void init(AttributeSet attrs) {
        ...
        if (attrs != null) {
            TypedArray obtainStyledAttributes = getContext().obtainStyledAttributes(attrs, C0207R.styleable.MotionLayout);
            int indexCount = obtainStyledAttributes.getIndexCount();
            boolean z = true;
            for (int i = 0; i < indexCount; i++) {
                int index = obtainStyledAttributes.getIndex(i);
                if (index == C0207R.styleable.MotionLayout_layoutDescription) {
                    this.mScene = new MotionScene(getContext(), this, obtainStyledAttributes.getResourceId(index, -1));
                } else if (index == C0207R.styleable.MotionLayout_currentState) {
                    this.mCurrentState = obtainStyledAttributes.getResourceId(index, -1);
                } else if (index == C0207R.styleable.MotionLayout_motionProgress) {
                    this.mTransitionGoalPosition = obtainStyledAttributes.getFloat(index, 0.0f);
                    this.mInTransition = true;
                } else if (index == C0207R.styleable.MotionLayout_applyMotionScene) {
                    z = obtainStyledAttributes.getBoolean(index, z);
                } else if (index == C0207R.styleable.MotionLayout_showPaths) {
                    if (this.mDebugPath == 0) {
                        this.mDebugPath = obtainStyledAttributes.getBoolean(index, false) ? 2 : 0;
                    }
                } else if (index == C0207R.styleable.MotionLayout_motionDebug) {
    ...
    public void updateStateAnimate(int stateId, ConstraintSet set, int duration) {
        if (this.mScene != null && this.mCurrentState == stateId) {
            updateState(C0207R.id.view_transition, getConstraintSet(stateId));
            setState(C0207R.id.view_transition, -1, -1);
            updateState(stateId, set);
            MotionScene.Transition transition = new MotionScene.Transition(-1, this.mScene, C0207R.id.view_transition, stateId);
