...
public class Easing {
    ...
    static class CubicEasing extends Easing {
        ...
        CubicEasing(String str) {
            ...
            this.f26x1 = Double.parseDouble(str.substring(indexOf + 1, indexOf2).trim());
            int i = indexOf2 + 1;
            int indexOf3 = str.indexOf(44, i);
            this.f28y1 = Double.parseDouble(str.substring(i, indexOf3).trim());
            int i2 = indexOf3 + 1;
            int indexOf4 = str.indexOf(44, i2);
            this.f27x2 = Double.parseDouble(str.substring(i2, indexOf4).trim());
            int i3 = indexOf4 + 1;
            this.f29y2 = Double.parseDouble(str.substring(i3, str.indexOf(41, i3)).trim());
        }
        ...
        private double getX(double d) {
            double d2 = 1.0d - d;
            double d3 = 3.0d * d2;
            return (this.f26x1 * d2 * d3 * d) + (this.f27x2 * d3 * d * d) + (d * d * d);
        }
        ...
        private double getY(double d) {
            double d2 = 1.0d - d;
            double d3 = 3.0d * d2;
            return (this.f28y1 * d2 * d3 * d) + (this.f29y2 * d3 * d * d) + (d * d * d);
        }

        /* JADX WARN: Unreachable blocks removed: 1, instructions: 1 */
        private double getDiffX(double d) {
            double d2 = 1.0d - d;
            double d3 = this.f26x1;
            double d4 = this.f27x2;
        ...
        private double getDiffY(double d) {
            double d2 = 1.0d - d;
            double d3 = this.f28y1;
            double d4 = this.f29y2;
            return (d2 * 3.0d * d2 * d3) + (d2 * 6.0d * d * (d4 - d3)) + (3.0d * d * d * (1.0d - d4));
        }
