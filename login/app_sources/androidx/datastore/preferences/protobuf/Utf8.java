...
final class Utf8 {
                ...
                if (b < 0) {
                    if (b < -32) {
                        ...
                        if (b >= -16) {
                            ...
                            if (b2 <= -65 && (((b << Ascii.f339FS) + (b2 + 112)) >> 30) == 0) {
                                int i5 = i4 + 1;
        ...
        int encodeUtf8(CharSequence charSequence, byte[] bArr, int i, int i2) {
            ...
            while (i4 < length) {
                ...
                if (charAt2 < c && j4 < j5) {
                    ...
                    long j13 = j4 + j;
                    UnsafeUtil.putByte(bArr, j4, (byte) ((charAt2 >>> '\f') | AddOnManager.StoreResultCode.INVALID_TRANSACTION_ID));
                    long j14 = j13 + j;
            ...
            while (i3 < length) {
                ...
                if (charAt2 >= c || position >= limit) {
                    if (charAt2 >= 2048 || position > limit - 2) {
                        ...
                        long j6 = position + 1;
                        UnsafeUtil.putByte(position, (byte) ((charAt2 >>> '\f') | AddOnManager.StoreResultCode.INVALID_TRANSACTION_ID));
                        long j7 = j6 + 1;
    ...
    private static class DecodeUtil {
        ...
        public static void handleTwoBytes(byte b, byte b2, char[] cArr, int i) throws InvalidProtocolBufferException {
            ...
            cArr[i] = (char) (((b & Ascii.f348US) << 6) | trailingByteValue(b2));
        }
        ...
        public static void handleThreeBytes(byte b, byte b2, byte b3, char[] cArr, int i) throws InvalidProtocolBufferException {
            ...
            cArr[i] = (char) (((b & Ascii.f345SI) << 12) | (trailingByteValue(b2) << 6) | trailingByteValue(b3));
        }

        /* JADX INFO: Access modifiers changed from: private */
        /* JADX WARN: Unreachable blocks removed: 1, instructions: 1 */
        public static void handleFourBytes(byte b, byte b2, byte b3, byte b4, char[] cArr, int i) throws InvalidProtocolBufferException {
            if (isNotTrailingByte(b2) || (((b << Ascii.f339FS) + (b2 + 112)) >> 30) != 0 || isNotTrailingByte(b3) || isNotTrailingByte(b4)) {
                throw InvalidProtocolBufferException.invalidUtf8();
            }
