...
public final class DebugProbesImpl {
    ...
    private final Function1<Boolean, Unit> getDynamicAttach() {
        ...
        try {
            ...
            m1440constructorimpl = Result.m1440constructorimpl((Function1) TypeIntrinsics.beforeCheckcastToFunctionOfArity(newInstance, 1));
        } catch (Throwable th) {
        ...
        return (Function1) m1440constructorimpl;
    }
    ...
    private final void build(Job job, Map<Job, DebugCoroutineInfoImpl> map, StringBuilder sb, String str) {
        ...
        if (debugCoroutineInfoImpl == null) {
            ...
            sb.append(str + getDebugString(job) + ", continuation is " + debugCoroutineInfoImpl.getState() + " at line " + ((StackTraceElement) CollectionsKt.firstOrNull((List) debugCoroutineInfoImpl.lastObservedStackTrace())) + '\n');
            str = str + '\t';
        }
    ...
    private final List<StackTraceElement> enhanceStackTraceWithThreadDumpImpl(String state, Thread thread, List<StackTraceElement> coroutineTrace) {
        ...
        try {
            Result.Companion companion = Result.INSTANCE;
            DebugProbesImpl debugProbesImpl = this;
            m1440constructorimpl = Result.m1440constructorimpl(thread.getStackTrace());
        } catch (Throwable th) {
            Result.Companion companion2 = Result.INSTANCE;
            m1440constructorimpl = Result.m1440constructorimpl(ResultKt.createFailure(th));
        }
        ...
        StackTraceElement[] stackTraceElementArr = (StackTraceElement[]) m1440constructorimpl;
    ...
    private final Pair<Integer, Integer> findContinuationStartIndex(int indexOfResumeWith, StackTraceElement[] actualTrace, List<StackTraceElement> coroutineTrace) {
        for (int i = 0; i < 3; i++) {
            ...
            if (findIndexOfFrame != -1) {
                return TuplesKt.m846to(Integer.valueOf(findIndexOfFrame), Integer.valueOf(i));
            }
        }
        return TuplesKt.m846to(-1, 0);
    }
