...
public final class ConcurrentLinkedListKt {
    ...
    public static final <S extends Segment<S>> Object findSegmentInternal(S s, long j, Function2<? super Long, ? super S, ? extends S> function2) {
        while (true) {
            if (s.id >= j && !s.isRemoved()) {
                return SegmentOrClosed.m3001constructorimpl(s);
            }
            Object nextOrClosed = s.getNextOrClosed();
            if (nextOrClosed == CLOSED) {
                return SegmentOrClosed.m3001constructorimpl(CLOSED);
            }
    ...
    public static final boolean moveForward$atomicfu(Object obj, AtomicReferenceFieldUpdater atomicReferenceFieldUpdater, Segment segment) {
        while (true) {
            ...
            if (UByte$$ExternalSyntheticBackport0.m852m(atomicReferenceFieldUpdater, obj, segment2, segment)) {
    ...
    public static final boolean moveForward$atomicfu$array(Object obj, AtomicReferenceArray atomicReferenceArray, int i, Segment segment) {
        while (true) {
            ...
            if (UByte$$ExternalSyntheticBackport0.m851m(atomicReferenceArray, i, segment2, segment)) {
                if (segment2.decPointers$kotlinx_coroutines_core()) {
                    segment2.remove();
                }
                return true;
        ...
        do {
            ...
            if (!SegmentOrClosed.m3006isClosedimpl(findSegmentInternal)) {
                ...
                while (true) {
                    Segment segment2 = (Segment) atomicReferenceFieldUpdater.get(obj);
                    z = true;
                    if (segment2.id >= m3004getSegmentimpl.id) {
                        break;
                    }
                    ...
                    if (UByte$$ExternalSyntheticBackport0.m852m(atomicReferenceFieldUpdater, obj, segment2, m3004getSegmentimpl)) {
                        if (segment2.decPointers$kotlinx_coroutines_core()) {
                            segment2.remove();
                        }
        ...
        do {
            ...
            if (!SegmentOrClosed.m3006isClosedimpl(findSegmentInternal)) {
                ...
                while (true) {
                    ...
                    if (UByte$$ExternalSyntheticBackport0.m851m(atomicReferenceArray, i, segment2, m3004getSegmentimpl)) {
                        if (segment2.decPointers$kotlinx_coroutines_core()) {
                            segment2.remove();
                        }
                    } else if (m3004getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
