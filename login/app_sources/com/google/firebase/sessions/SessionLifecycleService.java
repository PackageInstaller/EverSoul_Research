    ...
    public static final class MessageHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            if (this.lastMsgTimeMs > msg.getWhen()) {
                Log.d(SessionLifecycleService.TAG, "Ignoring old message from " + msg.getWhen() + " which is older than " + this.lastMsgTimeMs + ClassUtils.PACKAGE_SEPARATOR_CHAR);
                return;
            }
            int i = msg.what;
            if (i == 1) {
        ...
        private final void handleForegrounding(Message msg) {
            Log.d(SessionLifecycleService.TAG, "Activity foregrounding at " + msg.getWhen() + ClassUtils.PACKAGE_SEPARATOR_CHAR);
        ...
        private final void sendSessionToClient(Messenger client, String sessionId) {
            try {
                ...
                Log.w(SessionLifecycleService.TAG, "Unable to push new session to " + client + ClassUtils.PACKAGE_SEPARATOR_CHAR, e);
            }
        }

        /* JADX WARN: Unreachable blocks removed: 1, instructions: 1 */
        private final boolean isSessionRestart(long foregroundTimeMs) {
            return foregroundTimeMs - this.lastMsgTimeMs > Duration.m2784getInWholeMillisecondsimpl(SessionsSettings.INSTANCE.getInstance().m1204getSessionRestartTimeoutUwyO8pc());
        }
    }
