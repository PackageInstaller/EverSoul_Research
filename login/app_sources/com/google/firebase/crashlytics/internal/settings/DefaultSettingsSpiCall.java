...
class DefaultSettingsSpiCall implements SettingsSpiCall {
    ...
    public JSONObject invoke(SettingsRequest settingsRequest, boolean z) {
        ...
        try {
            ...
            this.logger.m390d("Requesting settings from " + this.url);
            this.logger.m396v("Settings query params were: " + queryParamsFor);
            return handleResponse(applyHeadersTo.execute());
        } catch (IOException e) {
            this.logger.m393e("Settings request failed.", e);
            return null;
        }
    }
    ...
    JSONObject handleResponse(HttpResponse httpResponse) {
        ...
        this.logger.m396v("Settings response code was: " + code);
        if (requestWasSuccessful(code)) {
            return getJsonObjectFrom(httpResponse.body());
        }
        this.logger.m392e("Settings request failed; (status: " + code + ") from " + this.url);
        return null;
    }
    ...
    private JSONObject getJsonObjectFrom(String str) {
        try {
            ...
            this.logger.m399w("Failed to parse settings JSON from " + this.url, e);
            this.logger.m398w("Settings response " + str);
            return null;
        }
    }
