...
class CrashlyticsController {
    ...
    synchronized void handleUncaughtException(final SettingsProvider settingsProvider, final Thread thread, final Throwable th, final boolean z) {
        Logger.getLogger().m390d("Handling uncaught exception \"" + th + "\" from thread " + thread.getName());
        ...
        try {
            Utils.awaitEvenIfOnMainThread(this.backgroundWorker.submitTask(new Callable<Task<Void>>() { // from class: com.google.firebase.crashlytics.internal.common.CrashlyticsController.2
                ...
                public Task<Void> call() throws Exception {
                        ...
                        if (CrashlyticsController.this.dataCollectionArbiter.isAutomaticDataCollectionEnabled()) {
                            ...
                            return settingsProvider.getSettingsAsync().onSuccessTask(executor, new SuccessContinuation<Settings, Void>() { // from class: com.google.firebase.crashlytics.internal.common.CrashlyticsController.2.1
                                ...
                                public Task<Void> then(Settings settings) throws Exception {
                                    ...
                                    Logger.getLogger().m398w("Received null app settings, cannot send reports at crash time.");
                    ...
                    Logger.getLogger().m392e("Tried to write a fatal exception while no session was open.");
                    return Tasks.forResult(null);
                }
            }));
        } catch (TimeoutException unused) {
            Logger.getLogger().m392e("Cannot send reports. Timed out while fetching settings.");
        } catch (Exception e) {
            Logger.getLogger().m393e("Error handling uncaught exception", e);
        }
    }
    ...
    private Task<Boolean> waitForReportAction() {
        if (this.dataCollectionArbiter.isAutomaticDataCollectionEnabled()) {
            Logger.getLogger().m390d("Automatic data collection is enabled. Allowing upload.");
            this.unsentReportsAvailable.trySetResult(false);
            return Tasks.forResult(true);
        }
        Logger.getLogger().m390d("Automatic data collection is disabled.");
        Logger.getLogger().m396v("Notifying that unsent reports are available.");
        ...
        Logger.getLogger().m390d("Waiting for send/deleteUnsentReports to be called.");
    ...
    boolean didCrashOnPreviousExecution() {
        ...
        Logger.getLogger().m396v("Found previous crash marker.");
    ...
    Task<Boolean> checkForUnsentReports() {
        if (!this.checkForUnsentReportsCalled.compareAndSet(false, true)) {
            Logger.getLogger().m398w("checkForUnsentReports should only be called once per execution.");
    ...
    Task<Void> submitAllReports(Task<Settings> task) {
        if (!this.reportingCoordinator.hasReportsToSend()) {
            Logger.getLogger().m396v("No crash reports are available to be sent.");
            this.unsentReportsAvailable.trySetResult(false);
            return Tasks.forResult(null);
        }
        Logger.getLogger().m396v("Crash reports are available to be sent.");
        return waitForReportAction().onSuccessTask(new C17364(task));
    }

    /* renamed from: com.google.firebase.crashlytics.internal.common.CrashlyticsController$4 */
    class C17364 implements SuccessContinuation<Boolean, Void> {
            ...
            return CrashlyticsController.this.backgroundWorker.submitTask(new Callable<Task<Void>>() { // from class: com.google.firebase.crashlytics.internal.common.CrashlyticsController.4.1
                ...
                public Task<Void> call() throws Exception {
                    if (!bool.booleanValue()) {
                        Logger.getLogger().m396v("Deleting cached crash reports...");
                        CrashlyticsController.deleteFiles(CrashlyticsController.this.listAppExceptionMarkerFiles());
                        CrashlyticsController.this.reportingCoordinator.removeAllReports();
                        CrashlyticsController.this.unsentReportsHandled.trySetResult(null);
                        return Tasks.forResult(null);
                    }
                    Logger.getLogger().m390d("Sending cached crash reports...");
                    CrashlyticsController.this.dataCollectionArbiter.grantDataCollectionPermission(bool.booleanValue());
                    final Executor executor = CrashlyticsController.this.backgroundWorker.getExecutor();
                    return C17364.this.val$settingsDataTask.onSuccessTask(executor, new SuccessContinuation<Settings, Void>() { // from class: com.google.firebase.crashlytics.internal.common.CrashlyticsController.4.1.1
                        /* JADX WARN: Unreachable blocks removed: 1, instructions: 1 */
                        @Override // com.google.android.gms.tasks.SuccessContinuation
                        public Task<Void> then(Settings settings) throws Exception {
                            if (settings != null) {
                            ...
                            Logger.getLogger().m398w("Received null app settings at app startup. Cannot send cached reports");
                            return Tasks.forResult(null);
                        }
                    });
                }
        ...
        this.backgroundWorker.submit(new Runnable() { // from class: com.google.firebase.crashlytics.internal.common.CrashlyticsController.6
            ...
            public void run() {
                ...
                if (currentSessionId != null) {
                    ...
                    Logger.getLogger().m398w("Tried to write a non-fatal exception while no session was open.");
                }
            }
        });
    }
    ...
    void logFatalException(Thread thread, Throwable th) {
        ...
        if (settingsProvider == null) {
            Logger.getLogger().m398w("settingsProvider not set");
        } else {
    ...
    void setCustomKey(String str, String str2) {
        try {
            ...
            Logger.getLogger().m392e("Attempting to set custom attribute with null key, ignoring.");
        }
    }
    ...
    boolean finalizeSessions(SettingsProvider settingsProvider) {
        ...
        if (isHandlingException()) {
            Logger.getLogger().m398w("Skipping session finalization because a crash has already occurred.");
            return Boolean.FALSE.booleanValue();
        }
        Logger.getLogger().m396v("Finalizing previously open sessions.");
        try {
            doCloseSessions(true, settingsProvider);
            Logger.getLogger().m396v("Closed all previously open sessions.");
            return true;
        } catch (Exception e) {
            Logger.getLogger().m393e("Unable to finalize previously open sessions.", e);
            return false;
        }
    }
    ...
    public void doOpenSession(String str, Boolean bool) {
        ...
        Logger.getLogger().m390d("Opening a new session with ID " + str);
    ...
    private void doCloseSessions(boolean z, SettingsProvider settingsProvider) {
        ...
        if (arrayList.size() <= z) {
            Logger.getLogger().m396v("No open sessions to be closed.");
            return;
        }
        ...
        if (settingsProvider.getSettingsSync().featureFlagData.collectAnrs) {
            ...
            Logger.getLogger().m396v("ANR feature disabled.");
        }
    ...
    void saveVersionControlInfo() {
        try {
            ...
            if (versionControlInfo != null) {
                ...
                Logger.getLogger().m394i("Saved version control info");
            }
        } catch (IOException e) {
            Logger.getLogger().m399w("Unable to save version control info", e);
        }
    }
    ...
    String getVersionControlInfo() throws IOException {
        ...
        Logger.getLogger().m390d("Read version control info");
    ...
    private InputStream getResourceAsStream(String str) {
        ...
        if (classLoader == null) {
            Logger.getLogger().m398w("Couldn't get Class Loader");
            return null;
        }
        ...
        Logger.getLogger().m394i("No version control information found");
        return null;
    }
    ...
    private void finalizePreviousNativeSession(String str) {
        Logger.getLogger().m396v("Finalizing native report for session " + str);
        ...
        if (nativeCoreAbsent(str, minidumpFile, applicationExitInto)) {
            Logger.getLogger().m398w("No native core present");
            return;
        }
        ...
        if (!nativeSessionDir.isDirectory()) {
            Logger.getLogger().m398w("Couldn't create directory to store native session files, aborting.");
            return;
        }
        ...
        Logger.getLogger().m390d("CrashlyticsController#finalizePreviousNativeSession");
    ...
    private static boolean nativeCoreAbsent(String str, File file, CrashlyticsReport.ApplicationExitInfo applicationExitInfo) {
        if (file == null || !file.exists()) {
            Logger.getLogger().m398w("No minidump data found for session " + str);
        }
        if (applicationExitInfo == null) {
            Logger.getLogger().m394i("No Tombstones data found for session " + str);
        }
    ...
    public void doWriteAppExceptionMarker(long j) {
        try {
            ...
            Logger.getLogger().m399w("Could not create app exception marker file.", e);
        }
    }
    ...
    public Task<Void> logAnalyticsAppExceptionEvents() {
        ...
        for (File file : listAppExceptionMarkerFiles()) {
            try {
                ...
                Logger.getLogger().m398w("Could not parse app exception timestamp from file " + file.getName());
            }
            file.delete();
        }
    ...
    private Task<Void> logAnalyticsAppExceptionEvent(final long j) {
        if (firebaseCrashExists()) {
            Logger.getLogger().m398w("Skipping logging Crashlytics event to Firebase, FirebaseCrash exists");
            return Tasks.forResult(null);
        }
        Logger.getLogger().m390d("Logging app exception event to Firebase Analytics");
    ...
    private void writeApplicationExitInfoEventIfRelevant(String str) {
        if (Build.VERSION.SDK_INT >= 30) {
            ...
            Logger.getLogger().m396v("No ApplicationExitInfo available. Session: " + str);
            return;
        }
        Logger.getLogger().m396v("ANR feature enabled, but device is API " + Build.VERSION.SDK_INT);
    }
}
