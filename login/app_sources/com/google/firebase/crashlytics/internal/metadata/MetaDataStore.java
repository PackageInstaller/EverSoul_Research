...
class MetaDataStore {
        ...
        try {
            try {
                ...
                try {
                    ...
                    Logger.getLogger().m399w("Error serializing user metadata.", e);
    ...
    public String readUserId(String str) {
        ...
        if (!userDataFileForSession.exists() || userDataFileForSession.length() == 0) {
            Logger.getLogger().m390d("No userId set for session " + str);
        ...
        try {
            try {
                ...
                Logger.getLogger().m390d("Loaded userId " + jsonToUserId + " for session " + str);
            ...
            e = e2;
            Logger.getLogger().m399w("Error deserializing user metadata.", e);
            safeDeleteCorruptFile(userDataFileForSession);
            CommonUtils.closeOrLog(fileInputStream, "Failed to close user metadata file.");
            return null;
        }
        ...
        try {
            try {
                ...
                try {
                    ...
                    e = e;
                    bufferedWriter = bufferedWriter2;
                    Logger.getLogger().m399w("Error serializing key/value metadata.", e);
        ...
        try {
            try {
                ...
                try {
                    ...
                    e = e;
                    bufferedWriter = bufferedWriter2;
                    Logger.getLogger().m399w("Error serializing rollouts state.", e);
                    safeDeleteCorruptFile(rolloutsStateForSession);
    ...
    private static List<RolloutAssignment> jsonToRolloutsState(String str) throws JSONException {
        ...
        for (int i = 0; i < jSONArray.length(); i++) {
            ...
            try {
                ...
                Logger.getLogger().m399w("Failed de-serializing rollouts state. " + string, e);
            }
        }
        return arrayList;
    }
    ...
    private static String rolloutsStateToJson(List<RolloutAssignment> list) {
        ...
        for (int i = 0; i < list.size(); i++) {
            try {
                ...
                Logger.getLogger().m399w("Exception parsing rollout assignment!", e);
            }
        }
    ...
    private static void safeDeleteCorruptFile(File file) {
        if (file.exists() && file.delete()) {
            Logger.getLogger().m394i("Deleted corrupt file: " + file.getAbsolutePath());
        }
    }
}
