...
public abstract class zzjd<T extends zzis> {
    ...
    static <T extends zzis> T zza(Class<T> cls) {
        String format;
        ...
        try {
            ...
            while (it.hasNext()) {
                try {
                    ...
                    Logger.getLogger(zzio.class.getName()).logp(Level.SEVERE, "com.google.protobuf.GeneratedExtensionRegistryLoader", "load", "Unable to load " + cls.getSimpleName(), (Throwable) e5);
                }
            }
            if (arrayList.size() == 1) {
