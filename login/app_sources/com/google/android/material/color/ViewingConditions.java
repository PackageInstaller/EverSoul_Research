package com.google.android.material.color;

/* loaded from: classes2.dex */
final class ViewingConditions {
    public static final ViewingConditions DEFAULT = make(ColorUtils.whitePointD65(), (float) ((ColorUtils.yFromLstar(50.0f) * 63.66197723675813d) / 100.0d), 50.0f, 2.0f, false);

    /* renamed from: aw */
    private final float f258aw;

    /* renamed from: c */
    private final float f259c;

    /* renamed from: fl */
    private final float f260fl;
    private final float flRoot;

    /* renamed from: n */
    private final float f261n;
    private final float nbb;

    /* renamed from: nc */
    private final float f262nc;
    private final float ncb;
    private final float[] rgbD;

    /* renamed from: z */
    private final float f263z;

    /* JADX WARN: Unreachable blocks removed: 1, instructions: 1 */
    public float getAw() {
        return this.f258aw;
    }

    /* JADX WARN: Unreachable blocks removed: 1, instructions: 1 */
    public float getN() {
        return this.f261n;
    }

    /* JADX WARN: Unreachable blocks removed: 1, instructions: 1 */
    public float getNbb() {
        return this.nbb;
    }

    /* JADX WARN: Unreachable blocks removed: 1, instructions: 1 */
    float getNcb() {
        return this.ncb;
    }

    /* JADX WARN: Unreachable blocks removed: 1, instructions: 1 */
    float getC() {
        return this.f259c;
    }

    /* JADX WARN: Unreachable blocks removed: 1, instructions: 1 */
    float getNc() {
        return this.f262nc;
    }

    /* JADX WARN: Unreachable blocks removed: 1, instructions: 1 */
    public float[] getRgbD() {
        return this.rgbD;
    }

    /* JADX WARN: Unreachable blocks removed: 1, instructions: 1 */
    float getFl() {
        return this.f260fl;
    }

    /* JADX WARN: Unreachable blocks removed: 1, instructions: 1 */
    public float getFlRoot() {
        return this.flRoot;
    }

    /* JADX WARN: Unreachable blocks removed: 1, instructions: 1 */
    float getZ() {
        return this.f263z;
    }

    /* JADX WARN: Unreachable blocks removed: 1, instructions: 1 */
    static ViewingConditions make(float[] fArr, float f, float f2, float f3, boolean z) {
        float lerp;
        float[][] fArr2 = Cam16.XYZ_TO_CAM16RGB;
        float f4 = fArr[0];
        float[] fArr3 = fArr2[0];
        float f5 = fArr3[0] * f4;
        float f6 = fArr[1];
        float f7 = f5 + (fArr3[1] * f6);
        float f8 = fArr[2];
        float f9 = f7 + (fArr3[2] * f8);
        float[] fArr4 = fArr2[1];
        float f10 = (fArr4[0] * f4) + (fArr4[1] * f6) + (fArr4[2] * f8);
        float[] fArr5 = fArr2[2];
        float f11 = (f4 * fArr5[0]) + (f6 * fArr5[1]) + (f8 * fArr5[2]);
        float f12 = (f3 / 10.0f) + 0.8f;
        if (f12 >= 0.9d) {
            lerp = MathUtils.lerp(0.59f, 0.69f, (f12 - 0.9f) * 10.0f);
        } else {
            lerp = MathUtils.lerp(0.525f, 0.59f, (f12 - 0.8f) * 10.0f);
        }
        float f13 = lerp;
        float exp = z ? 1.0f : (1.0f - (((float) Math.exp(((-f) - 42.0f) / 92.0f)) * 0.2777778f)) * f12;
        double d = exp;
        if (d > 1.0d) {
            exp = 1.0f;
        } else if (d < 0.0d) {
            exp = 0.0f;
        }
        float[] fArr6 = {(((100.0f / f9) * exp) + 1.0f) - exp, (((100.0f / f10) * exp) + 1.0f) - exp, (((100.0f / f11) * exp) + 1.0f) - exp};
        float f14 = 1.0f / ((5.0f * f) + 1.0f);
        float f15 = f14 * f14 * f14 * f14;
        float f16 = 1.0f - f15;
        float cbrt = (f15 * f) + (0.1f * f16 * f16 * ((float) Math.cbrt(f * 5.0d)));
        float yFromLstar = ColorUtils.yFromLstar(f2) / fArr[1];
        double d2 = yFromLstar;
        float sqrt = ((float) Math.sqrt(d2)) + 1.48f;
        float pow = 0.725f / ((float) Math.pow(d2, 0.2d));
        float pow2 = (float) Math.pow(((fArr6[2] * cbrt) * f11) / 100.0d, 0.42d);
        float[] fArr7 = {(float) Math.pow(((fArr6[0] * cbrt) * f9) / 100.0d, 0.42d), (float) Math.pow(((fArr6[1] * cbrt) * f10) / 100.0d, 0.42d), pow2};
        float f17 = fArr7[0];
        float f18 = fArr7[1];
        return new ViewingConditions(yFromLstar, ((((f17 * 400.0f) / (f17 + 27.13f)) * 2.0f) + ((f18 * 400.0f) / (f18 + 27.13f)) + (((400.0f * pow2) / (pow2 + 27.13f)) * 0.05f)) * pow, pow, pow, f13, f12, fArr6, cbrt, (float) Math.pow(cbrt, 0.25d), sqrt);
    }

    /* JADX WARN: Unreachable blocks removed: 1, instructions: 1 */
    private ViewingConditions(float f, float f2, float f3, float f4, float f5, float f6, float[] fArr, float f7, float f8, float f9) {
        this.f261n = f;
        this.f258aw = f2;
        this.nbb = f3;
        this.ncb = f4;
        this.f259c = f5;
        this.f262nc = f6;
        this.rgbD = fArr;
        this.f260fl = f7;
        this.flRoot = f8;
        this.f263z = f9;
    }
}
