...
final class StandardMutableNetwork<N, E> extends StandardNetwork<N, E> implements MutableNetwork<N, E> {
    ...
    public boolean addEdge(N n, N n2, E e) {
        ...
        if (containsEdge(e)) {
            ...
            EndpointPair m329of = EndpointPair.m329of(this, n, n2);
            Preconditions.checkArgument(incidentNodes.equals(m329of), "Edge %s already exists between the following nodes: %s, so it cannot be reused to connect the following nodes: %s.", e, incidentNodes, m329of);
            return false;
        }
    ...
    private NetworkConnections<N, E> newConnections() {
        if (isDirected()) {
            if (allowsParallelEdges()) {
                return DirectedMultiNetworkConnections.m326of();
            }
            return DirectedNetworkConnections.m327of();
        }
        if (allowsParallelEdges()) {
            return UndirectedMultiNetworkConnections.m332of();
        }
        return UndirectedNetworkConnections.m333of();
    }
}
