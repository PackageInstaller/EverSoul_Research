...
public final class ShareApi {
    ...
    private void addCommonParameters(final Bundle bundle, ShareContent shareContent) {
        ...
        try {
            ...
            if (!Utility.isNullOrEmpty(peopleIds)) {
                bundle.putString(Constants.TAGS, TextUtils.join(", ", peopleIds));
            }
    ...
    private void shareLinkContent(final ShareLinkContent linkContent, final FacebookCallback<Sharer.Result> callback) {
        ...
        try {
            GraphRequest.Callback callback2 = new GraphRequest.Callback() { // from class: com.facebook.share.ShareApi.2
                ...
                public void onCompleted(GraphResponse response) {
                    JSONObject jSONObject = response.getJSONObject();
                    ShareInternalUtility.invokeCallbackWithResults(callback, jSONObject == null ? null : jSONObject.optString("id"), response);
                }
            };
            ...
            new GraphRequest(AccessToken.getCurrentAccessToken(), getGraphPath(com.kakao.sdk.template.Constants.TYPE_FEED), bundle, HttpMethod.POST, callback2).executeAsync();
        } catch (Throwable th) {
            CrashShieldHandler.handleThrowable(th, this);
        }
    }
    ...
    private Bundle getSharePhotoCommonParameters(SharePhoto photo, SharePhotoContent photoContent) throws JSONException {
        ...
        try {
            ...
            if (!parameters.containsKey(Constants.TAGS) && !Utility.isNullOrEmpty(photoContent.getPeopleIds())) {
                ...
                if (!Utility.isNullOrEmpty(peopleIds)) {
                    ...
                    parameters.putString(Constants.TAGS, jSONArray.toString());
                }
            }
