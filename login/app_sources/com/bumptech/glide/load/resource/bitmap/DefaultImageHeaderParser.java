...
public final class DefaultImageHeaderParser implements ImageHeaderParser {
    ...
    static final byte[] JPEG_EXIF_SEGMENT_PREAMBLE_BYTES = JPEG_EXIF_SEGMENT_PREAMBLE.getBytes(Charset.forName(y.٬ݯح׭٩(576116366)));
    ...
    private int getOrientation(Reader reader, ArrayPool arrayPool) throws IOException {
        ...
        String str = y.٬ݯح׭٩(576044118);
        if (!handles) {
            if (Log.isLoggable(str, 3)) {
                Log.d(str, "Parser doesn't handle magic number: " + uInt16);
            }
            return -1;
        }
    ...
    private int parseExifSegment(Reader reader, byte[] bArr, int i) throws IOException {
        ...
        if (read != i) {
            if (Log.isLoggable(str, 3)) {
                Log.d(str, y.ݬֲ֮ܲت(1513056111) + i + ", actually read: " + read);
            }
            return -1;
        }
    ...
    private int moveToExifSegmentAndGetLength(Reader reader) throws IOException {
        ...
        do {
            ...
            str = y.٬ݯح׭٩(576044118);
            if (uInt82 != 255) {
                if (Log.isLoggable(str, 3)) {
                    Log.d(str, y.ۮڭڭܬި(862064683) + ((int) uInt82));
                }
                return -1;
            }
        ...
        if (Log.isLoggable(str, 3)) {
            Log.d(str, y.ٲٴݴ״ٰ(1781753512) + ((int) uInt8) + y.٬ݯح׭٩(576044782) + uInt16 + y.ۮڭڭܬި(862065227) + skip);
        }
        return -1;
    }
    ...
    private static int parseExifSegment(RandomAccessReader randomAccessReader) {
        ...
        if (int16 == INTEL_TIFF_MAGIC_NUMBER) {
            ...
            if (Log.isLoggable(str, 3)) {
                Log.d(str, y.ٲٴݴ״ٰ(1781754712) + ((int) int16));
            }
            byteOrder = ByteOrder.BIG_ENDIAN;
        }
        ...
        for (int i = 0; i < int162; i++) {
            ...
            if (int163 == ORIENTATION_TAG_TYPE) {
                ...
                if (int164 < 1 || int164 > 12) {
                    if (Log.isLoggable(str, 3)) {
                        Log.d(str, y.دײܮڳܯ(2052017765) + ((int) int164));
                    }
                } else {
                    int int322 = randomAccessReader.getInt32(calcTagOffset + 4);
                    if (int322 < 0) {
                        if (Log.isLoggable(str, 3)) {
                            Log.d(str, y.֬ڱܱײٮ(-1159321543));
                        }
                    } else {
                        boolean isLoggable = Log.isLoggable(str, 3);
                        String str2 = y.ݮڮֲڭܩ(-628997716);
                        if (isLoggable) {
                            Log.d(str, y.ٲٴݴ״ٰ(1781754176) + i + str2 + ((int) int163) + y.ٲٴݴ״ٰ(1781754328) + ((int) int164) + y.ٴسسݬߨ(1392932706) + int322);
                        }
                        int i2 = int322 + BYTES_PER_FORMAT[int164];
                        if (i2 > 4) {
                            if (Log.isLoggable(str, 3)) {
                                Log.d(str, y.٬ݯح׭٩(576041982) + ((int) int164));
                            }
                        } else {
                            int i3 = calcTagOffset + 8;
                            if (i3 < 0 || i3 > randomAccessReader.length()) {
                                if (Log.isLoggable(str, 3)) {
                                    Log.d(str, y.ݬֲ֮ܲت(1513052375) + i3 + str2 + ((int) int163));
                                }
                            } else if (i2 < 0 || i2 + i3 > randomAccessReader.length()) {
                                if (Log.isLoggable(str, 3)) {
                                    Log.d(str, y.ݮڮֲڭܩ(-628996628) + ((int) int163));
                                }
                            } else {
    ...
    private static final class StreamReader implements Reader {
        ...
        /* renamed from: is */
        private final InputStream f165is;

        /* JADX WARN: Unreachable blocks removed: 1, instructions: 1 */
        StreamReader(InputStream inputStream) {
            this.f165is = inputStream;
        }

        /* JADX WARN: Unreachable blocks removed: 1, instructions: 1 */
        @Override // com.bumptech.glide.load.resource.bitmap.DefaultImageHeaderParser.Reader
        public int getUInt16() throws IOException {
            return ((this.f165is.read() << 8) & MotionEventCompat.ACTION_POINTER_INDEX_MASK) | (this.f165is.read() & 255);
        }

        /* JADX WARN: Unreachable blocks removed: 1, instructions: 1 */
        @Override // com.bumptech.glide.load.resource.bitmap.DefaultImageHeaderParser.Reader
        public short getUInt8() throws IOException {
            return (short) (this.f165is.read() & 255);
        }
        ...
        public long skip(long j) throws IOException {
            ...
            long j2 = j;
            while (j2 > 0) {
                long skip = this.f165is.skip(j2);
                if (skip <= 0) {
                    if (this.f165is.read() == -1) {
                        break;
                    }
                    skip = 1;
                }
        ...
        public int read(byte[] bArr, int i) throws IOException {
            int i2 = i;
            while (i2 > 0) {
                int read = this.f165is.read(bArr, i - i2, i2);
                if (read == -1) {
                    break;
                }
                i2 -= read;
