...
public class ByteBufferGifDecoder implements ResourceDecoder<ByteBuffer, GifDrawable> {
    ...
    private GifDrawableResource decode(ByteBuffer byteBuffer, int i, int i2, GifHeaderParser gifHeaderParser, Options options) {
        String str = y.دײܮڳܯ(2051968165);
        String str2 = y.ٲٴݴ״ٰ(1781698728);
        ...
        try {
            ...
            if (parseHeader.getNumFrames() > 0 && parseHeader.getStatus() == 0) {
                ...
                if (Log.isLoggable(str2, 2)) {
                    Log.v(str2, str + LogTime.getElapsedMillis(logTime));
                }
    ...
    private static int getSampleSize(GifHeader gifHeader, int i, int i2) {
        ...
        String str = y.ٲٴݴ״ٰ(1781698728);
        if (Log.isLoggable(str, 2) && max > 1) {
            StringBuilder append = new StringBuilder(y.ݮڮֲڭܩ(-628900836)).append(max).append(y.֬ڱܱײٮ(-1159211919)).append(i);
            String str2 = y.ٴسسݬߨ(1392956602);
            Log.v(str, append.append(str2).append(i2).append("], actual dimens: [").append(gifHeader.getWidth()).append(str2).append(gifHeader.getHeight()).append("]").toString());
        }
        return max;
    }
