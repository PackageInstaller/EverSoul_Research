...
public class HttpUrlFetcher implements DataFetcher<InputStream> {
        ...
        try {
            try {
                ...
                if (Log.isLoggable(str, 3)) {
                    Log.d(str, "Failed to load data for url", e);
                }
            ...
            if (Log.isLoggable(str, 2)) {
                sb = new StringBuilder("Finished http url fetcher fetch in ");
                Log.v(str, sb.append(LogTime.getElapsedMillis(logTime)).toString());
            }
        } catch (Throwable th) {
            if (Log.isLoggable(str, 2)) {
                Log.v(str, "Finished http url fetcher fetch in " + LogTime.getElapsedMillis(logTime));
            }
            throw th;
        }
    }
    ...
    private InputStream loadDataWithRedirects(URL url, int i, URL url2, Map<String, String> map) throws IOException {
        ...
        String headerField = this.urlConnection.getHeaderField(y.ٲٴݴ״ٰ(1781635520));
    ...
    private InputStream getStreamForSuccessfulRequest(HttpURLConnection httpURLConnection) throws IOException {
        if (TextUtils.isEmpty(httpURLConnection.getContentEncoding())) {
            ...
            String str = y.ٲٴݴ״ٰ(1781763480);
            if (Log.isLoggable(str, 3)) {
                Log.d(str, y.ٲٴݴ״ٰ(1781763096) + httpURLConnection.getContentEncoding());
            }
