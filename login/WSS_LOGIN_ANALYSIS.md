...
**底层库**: OkHttp 的 WebSocket 实现
- 文件: `okhttp3/internal/p034ws/RealWebSocket.java`
- 支持 `permessage-deflate` 扩展

**连接特性**:
```java
// okhttp3/internal/p034ws/RealWebSocket.java
Request build2 = this.originalRequest.newBuilder()
...
data = [
    "/v1/oauth/login",
    {"txNo": 123456},
    {"appId": "your-app-id", "playerId": "player-123"}
]
...
print(f"连接 URL: wss://server.com/session?m={encoded}")
```

#### 步骤 4: 建立 WebSocket 连接

```
wss://server.com/session?m=eJw7LCkpKSgJzivNSy_JzM_TLy4p...
```

#### 步骤 5: 发送消息（未压缩）

```json
["/v1/oauth/login",{"txNo":123456},{"appId":"your-app-id","playerId":"player-123","idpCode":"kakao","accessToken":"eyJ..."}]
```
...
### 示例 2: 解码从 Unity 传来的 Base64 数据

```python
import base64
import json

# Unity 传来的 Base64 编码数据
encoded_data = "WyIvdjEvb2F1dGgvbG9naW4iLHsidHhObyI6MTIzNDU2fSx7ImFwcElkIjoieW91ci1hcHAtaWQifV0="
...
print(f"URI: {request_data[0]}")
print(f"Header: {request_data[1]}")
print(f"Body: {request_data[2]}")
```

### 示例 3: 使用 Python 模拟完整流程
...
async def kakao_login_simulation():
    # 1. 构造请求
    request = [
        "/v1/oauth/login",
        {"txNo": 123456},
    ...
    # 3. 建立 WebSocket 连接
    url = f"wss://your-server.com/session?m={encoded}"
...
# 运行
asyncio.run(kakao_login_simulation())
```
...
| 场景 | 阈值 | 处理方式 |
|------|------|---------|
| Unity → Native 响应 | 300,000 字节 (约 293 KB) | 超过则写入文件 |
| WebSocket 连接参数 | 无限制 | 始终压缩+编码 |
| WebSocket 消息体 | 无限制 | 不压缩，直接发送 JSON |

### B. Base64 编码标志

```java
// Android Base64 类的标志位
Base64.DEFAULT        = 0   // 标准 Base64，带换行符
Base64.NO_WRAP        = 2   // 无换行符
Base64.URL_SAFE       = 8   // URL 安全字符集
Base64.URL_SAFE | NO_WRAP = 10  // URL 安全 + 无换行符
```
...
| 类型 | 默认超时 | 配置位置 |
|------|---------|---------|
| Session 连接 | 10000 ms (10秒) | `SessionService.sessionTimeout` |
| Session 请求 | 10000 ms (10秒) | `SessionService.sessionTimeout` |
| 游戏 Session | 可配置 | `GameSessionService.sessionTimeout` |
| 自定义超时 | `request.setTimeout()` | 每个请求可单独设置 |
...
1. **Unity ↔ Native**: Base64 编码，大数据使用文件
2. **WSS 连接**: Deflate 压缩 + URL-Safe Base64 编码的连接参数
3. **WSS 消息**: 未压缩的 JSON 字符串
4. **请求匹配**: 通过 `transactionNo` 实现异步请求/响应匹配
5. **并发支持**: 使用 `syncRequestMap` 支持多个并发请求
...
**文档版本**: 1.0  
**分析日期**: 2025-10-18  
**SDK版本**: Kakao Game SDK (反编译版本)  
**分析工具**: JADX, Android Studio  

---

## 参考资料

- [OkHttp WebSocket 文档](https://square.github.io/okhttp/4.x/okhttp/okhttp3/-web-socket/)
