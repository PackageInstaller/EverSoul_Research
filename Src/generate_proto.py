...
def parse_cs_file_from_types(proto_type, file_path):
    ...
    namespace_pattern = re.compile(
        rf"namespace\s+{re.escape(proto_type)}\s*{{(.*?)\n}}", re.DOTALL
    )
    combined_content = ""
        ...
        for field_number_match in field_number_matches:
            ...
            if not field_type:
                ...
                repeated_match = re.search(
                    repeated_field_pattern, class_content, re.IGNORECASE
                )
                ...
                if repeated_match:
                    ...
                    normal_field_pattern = (
                        rf"private\s+([^;]+?)\s+{re.escape(field_name.lower())}_"
                    )
                    normal_match = re.search(
                        normal_field_pattern, class_content, re.IGNORECASE
                    )
    ...
    with open(output_file, "w", encoding="utf-8") as f:
        ...
        for message_name, fields in message_types.items():
            ...
            for field_name, field_number, field_type, is_repeated in fields:
                proto_field_type = cs_type_to_proto_type(
                    field_type, message_types.keys()
                )

                repeated = "repeated " if is_repeated else ""

                f.write(
                    f"  {repeated}{proto_field_type} {field_name} = {field_number};\n"
                )
...
def process_types_file(proto_type, file_path, output_dir):
    ...
    namespace, message_types, enum_types = parse_cs_file_from_types(
        proto_type, file_path
    )
    ...
    for message_name in message_types:
        ...
        for field_name, field_number, field_type, is_repeated in message_types[
            message_name
        ]:
            for enum_name in enum_types:
                if (
                    enum_name == field_type
                    or f"E_{field_type}" == enum_name
                    or field_type == f"E_{enum_name}"
                ):
...
def main():
    ...
    os.chdir(script_dir)
    server = "Global"
    response_proto_dir = f"../ProtocolBuffers/{server}/Response"
    request_proto_dir = f"../ProtocolBuffers/{server}/Request"
    eschatpb_proto_dir = f"../ProtocolBuffers/{server}/ESChat"
