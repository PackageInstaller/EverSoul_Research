    ...
    bool decryptFiles(const std::vector<fs::path> &files, const std::vector<uint8_t> &key, const std::vector<uint8_t> &iv)
    {
        ...
        for (const auto &file : files)
        {
            ...
            if (!decryptFileInPlace(file, current_file, total_files, key, iv))
            {
                ...
                std::println("\033[33m警告: {} 解密失败，跳过该文件\033[0m", file.filename().string());
                // 继续处理其他文件，不要立即返回
            }
            ProgressDisplay::update("解密进度", current_file, total_files,
        ...
        if (failed_count > 0)
        {
            std::println("\033[33m数据表解密完成，但有 {} 个文件失败\033[0m", failed_count);
            std::println("\033[33m失败的文件:\033[0m");
            ...
            {
                std::println("\033[31m失败文件过多，可能存在严重问题\033[0m");
                return false;
            }
